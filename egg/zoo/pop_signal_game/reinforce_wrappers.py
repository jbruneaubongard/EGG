...
from egg.zoo.pop_signal_game.interaction import LoggingStrategy, Interaction
...
class SymbolPopGameReinforce(nn.Module):
    """
    A single-symbol population game implemented with Reinforce.
    """

    def __init__(
        self,
        senders: list[nn.Module],
        receivers: list[nn.Module],
        idx_list,
        loss: Callable,
        ...
        :param receiver_entropy_coeff: The entropy regularization coefficient for Receiver
        ...
        """
        super(SymbolPopGameReinforce, self).__init__()
    ...
    def forward(
        ...
        for sender_idx, receiver_idx in self.idx_list:
            sender = ReinforceWrapper(self.senders[sender_idx])
            receiver = ReinforceWrapper(self.receivers[receiver_idx])

            message, sender_log_prob, sender_entropy = sender(sender_input, aux_input)
            receiver_output, receiver_log_prob, receiver_entropy = receiver(message, receiver_input, aux_input)
            ...
            interaction = logging_strategy.filtered_interaction(
                ...
                training=torch.tensor([self.training]),
                sender_idx=torch.tensor([sender_idx]),
                receiver_idx=torch.tensor([receiver_idx]),
            ...
            list_full_loss.append(full_loss)
            list_interactions.append(interaction)
            
            all_interactions = Interaction.from_iterable(list_interactions)

        return sum(list_full_loss), all_interactions
