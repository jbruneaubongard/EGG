...
class SymbolPopGameGS(nn.Module):
    """
    Implements one-symbol population game. The loss must be differentiable wrt the parameters of the agents.
    ...
    def __init__(
        ...
        """
        super(SymbolPopGameGS, self).__init__()
    ...
    def forward(
        ...
        for sender_idx, receiver_idx in self.idx_list:
            sender = GumbelSoftmaxWrapper(self.senders[sender_idx], temperature=self.temperature)
        ...
        return sum(list_loss), all_interactions
