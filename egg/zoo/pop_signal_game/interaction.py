...
from egg.zoo.pop_signal_game.utils import NumpyEncoder


@dataclass(repr=True, eq=True)
class LoggingStrategy:
    store_sender_idx: bool = True
    store_receiver_idx: bool = True
    ...
    def filtered_interaction(
        self,
        epoch: Optional[torch.Tensor],
        turn: Optional[torch.Tensor],
        training: Optional[torch.Tensor],
        sender_idx: Optional[torch.Tensor],
        receiver_idx: Optional[torch.Tensor],
        ...
        return Interaction(
            ...
            turn=turn,
            training=training,
            sender_idx=sender_idx if self.store_sender_idx else None,
            receiver_idx=receiver_idx if self.store_receiver_idx else None,
...
class Interaction:
    ...
    def to(self, *args, **kwargs) -> "Interaction":
        ...
        self.epoch = _to(self.epoch)
        self.turn = _to(self.turn)
        self.training = _to(self.training)
        self.sender_idx = _to(self.sender_idx)
        self.receiver_idx = _to(self.receiver_idx)
    ...
    def from_iterable(interactions: Iterable["Interaction"]) -> "Interaction":
        ...
        return Interaction(
            epoch=torch.tensor([x.epoch for x in interactions]) if not isinstance(interactions[0].epoch, torch.Tensor) \
                else _check_cat([x.epoch for x in interactions]),
            turn=torch.tensor([x.turn for x in interactions]) if not isinstance(interactions[0].turn, torch.Tensor) \
                else _check_cat([x.turn for x in interactions]),
            training=torch.tensor([x.training for x in interactions]) if not isinstance(interactions[0].training, torch.Tensor) \
                else _check_cat([x.training for x in interactions]),
            sender_idx=torch.tensor([x.sender_idx for x in interactions]) if not isinstance(interactions[0].sender_idx, torch.Tensor) \
                else _check_cat([x.sender_idx for x in interactions]),
            receiver_idx=torch.tensor([x.receiver_idx for x in interactions]) if not isinstance(interactions[0].receiver_idx, torch.Tensor) \
                else _check_cat([x.receiver_idx for x in interactions]),
    ...
    def empty() -> "Interaction":
        return Interaction(None, None, None, None, None, None, None, {}, None, None, None, {})
    ...
    def save_to_json(self, path, id):
        ...
        for k,v in features['aux'].items():
            if isinstance(v, torch.Tensor):
                features['aux'][k] = v.cpu().numpy()

        for k,v in features.items():
            if isinstance(v, torch.Tensor):
                features[k] = v.cpu().numpy()
    
        with open(path + "/" + id + ".json", "w") as outfile:
            json.dump(features, outfile, cls=NumpyEncoder, indent=2)
        return None
